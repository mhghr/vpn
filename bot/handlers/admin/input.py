...
async def handle_admin_input(message: Message):
            ...
            try:
                ...
                try:
                    ...
                    if not servers:
                        ...
                        return
                    prefix = "org_create_acc_server_" if state.get("scope") == "org" else "create_acc_custom_server_"
                    await message.answer("سرور را برای ساخت اکانت انتخاب کنید:", reply_markup=get_plan_server_select_keyboard(servers, prefix), parse_mode="HTML")
                finally:
                    db.close()
            except ValueError:
                await message.answer("❌ لطفاً عدد معتبر وارد کنید.", parse_mode="HTML")
    ...
    if user_id in admin_plan_state and admin_plan_state[user_id].get("action") == "edit_org_price":
        ...
        try:
            ...
            if val <= 0:
                raise ValueError
            target_user = db.query(User).filter(User.id == int(state.get("target_user_id"))).first()
            if not target_user:
                await message.answer("❌ کاربر یافت نشد.", parse_mode="HTML")
                return
            target_user.org_price_per_gb = val
            db.commit()
            await message.answer("✅ هزینه هر گیگ بروزرسانی شد.", parse_mode="HTML")
        except ValueError:
            await message.answer("❌ لطفاً عدد صحیح معتبر وارد کنید.", parse_mode="HTML")
            return
        finally:
            db.close()
